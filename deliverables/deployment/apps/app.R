...
# data_comments <- data %>% 
#   select(listing_id,comments,review_scores_rating,neighbourhood_cleansed,neighbourhood_group_cleansed) %>%
...
data_comments <- read_csv('data/data_comments.csv')
data_count <- read_csv('data/data_count.csv')
data_count2 <- read_csv('data/data_count2.csv')
bigram_data_count <- read_csv('data/bigram_data_count.csv')
...
subregion_data <- read_csv('data/subregion_data.csv')
                ...
                tabPanel("Map",
                         fluidPage(
                           ...
                           sidebarLayout(
                             ...
                             mainPanel(
                               withSpinner(leafletOutput('leaf_map', height = 600, width = 900),type = 7, color = "#FF5A5F", size = 2),
                               conditionalPanel(
                                 condition = "input.map_type == 'Choropleth map'",
                                 withSpinner(DT::dataTableOutput('szTable'),type = 7, color = "#FF5A5F", size = 2)
                               )
                         ...
                         fluidPage(
                           ...
                           mainPanel( 
                             tabPanel("Topic Visualisation",
                                      withSpinner(plotOutput('visChart'),type = 7, color = "#FF5A5F"))
                           )
                         )
                ),
                           ...
                           box(width=12, height=500, solidHeader = F,
                               tabsetPanel(type = 'pills',
                                           ...
                                           tabPanel("Bi Directional Network", 
                                                    ...
                                                    withSpinner(plotOutput("network_plot1"),type = 7, color = "#FF5A5F", size = 2)),
...
server <- function(input, output) {
  ...
  output$word_cloud_plot <- renderWordcloud2({
    ...
    if(input$word_cloud_gram == "Uni-gram"){
      ...
      wordcloud2(data = d2, size=0.8, minSize = 0.0, fontWeight = 'bold', 
                 ellipticity = 0.65)
